...
public class Odometry_Test extends LinearOpMode
{
    @Override public void runOpMode()
    {
        ...
        Trajectory traj2 = drive.trajectoryBuilder(traj1.end(), -90)
                .lineToLinearHeading(new Pose2d(-36,30, Math.toRadians(-180))) //X
                .build();
        Trajectory traj3 = drive.trajectoryBuilder(traj2.end(), -90)
                .lineTo(new Vector2d())
                .build();
        Trajectory traj4 = drive.trajectoryBuilder(traj3.end(), -188)
                .lineTo(new Vector2d(-48,64))
                .build();
        Trajectory traj5 = drive.trajectoryBuilder(traj4.end(), -180)
                .lineTo(new Vector2d(-43,28))
                .build();
        Trajectory traj6 = drive.trajectoryBuilder(traj5.end(), -180)
                .lineTo(new Vector2d(-60,64))
                .build();
        Trajectory traj7 = drive.trajectoryBuilder(traj6.end(), -180)
                .lineTo(new Vector2d(-55,28))
                .build();
        Trajectory traj8 = drive.trajectoryBuilder(traj7.end(), -180)
